...
public class RegisterBlazeDsTemplates implements IRegisterTemplates {
	
	private final static SubsValueType FOLDER = SubsValueType.FOLDER;
	
	@Override
	public String getName() {
		return BlazeDsTemplates.BLAZEDS_CARTRIDGE_NAME;
	}
	...
	public List<TemplatesBranch> getTemplateBranches() {
		...
		result.add(makeBranch(BlazeDsTemplates.BLAZEDS_BRANCH_NAME, getDescription()));
	...
	private TemplatesBranch makeBranch(String aName, String aDescr) {
		...
		for (ITemplate template : new BlazeDsTemplates().getAllTemplates()) {
	...
	public Collection<SubsValue> getSubstituteValues() {
		...
		result.add(makeSubsValue(BlazeDsTemplates.BLAZEDS_BACKEND, FOLDER, "target directory ", 3, null ));
		return result;
	}
	...
	private SubsValue makeSubsValue(String name, SubsValueType type, String help, 
		...
		return RegisterTemplatesHelper.makeSubsValue(name, type, name, help, rank, aDefaultValue, 
				new String[] {BlazeDsTemplates.BLAZEDS_CARTRIDGE_NAME});
	}
